...
assert.doesNotMatch(brokenHtml + solutionHtml, /<script/i);
...
assert.match(brokenCss, /\.release-row:nth-child\(2\)/);
assert.match(brokenCss, /:is\(\.action, #legacy-action\)/);
assert.match(brokenCss, /\.status-error:has\(\.status-panel\)/);
...
assert.match(brokenCss, /:scope > \.scope-row/);
...
assert.match(solutionCss, /\.scope-zone > \.scope-row/);
...
console.log("✓ KP008 provides a static eleven-fault selector lab with no learner-facing JavaScript.");
