...
| RE-KP044 | State 不可直接修改 | 1. 只读快照 2. Setter 3. 直接赋值问题 | `kp044-state-immutability/` | 待生成 |
| RE-KP045 | 对象状态不可变更新 | 1. Object State 2. Spread 3. 替换对象 | `kp045-object-state-updates/` | 待生成 |
| RE-KP046 | 数组状态不可变更新 | 1. Array State 2. map/filter/spread 3. 避免原地修改 | `kp046-array-state-updates/` | 待生成 |
| RE-KP047 | 嵌套状态更新与结构设计 | 1. Nested State 2. 多层复制 3. 扁平化思维 | `kp047-nested-state-design/` | 待生成 |
| RE-KP048 | 函数式更新 | 1. Updater Function 2. Previous State 3. 连续更新 | `kp048-functional-state-updates/` | 待生成 |
| RE-KP049 | 惰性初始化 | 1. Initializer Function 2. 首次初始化 3. 成本边界 | `kp049-lazy-state-initialization/` | 待生成 |
| RE-KP050 | 避免把可推导值存入 State | 1. Derived Value 2. Single Source 3. 冗余 State | `kp050-avoid-redundant-state/` | 待生成 |

## 当前进度

- Chapter 05：**3 / 10**
- 下一知识点：**RE-KP044：State 不可直接修改**
